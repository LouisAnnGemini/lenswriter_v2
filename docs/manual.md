...
*   **归档章节 (Archive)**：
    *   **显示逻辑**：归档图标默认隐藏，当鼠标悬停在章节标题上时才会显示。
    *   点击章节右侧的**归档图标**。
...
*   **排序角色**：拖动角色卡片可以调整顺序。
*   **删除角色**：在角色详情页的姓名右侧，点击出现的**垃圾桶图标**。系统会显示内联确认提示，点击 **✓ (确认)** 将彻底删除该角色，并自动清理其在所有场景和时间轴事件中的关联信息。
...
在顶部的 **Timeline** 标签页中，您可以管理故事中的客观事件，并以时间顺序或场景蒙太奇的方式进行梳理。

### 5.1 时间轴视图 (Chronology View)

*   **搜索与筛选**：
    *   **搜索**：使用顶部的搜索框，通过标题或描述快速查找事件。
    *   **多维筛选**：支持按角色、地点、颜色和自定义标签进行多选筛选。
    *   **筛选管理**：选中的筛选条件会以“标签”形式显示，点击 `X` 可快速移除，点击“Clear all”可重置所有筛选。
*   **添加事件**：
    *   点击右上角的 **"+ New Event"** 按钮，在弹出的独立窗口中输入时间戳、标题、地点并关联标签。
*   **排序**：在时间轴中，按住事件左侧的抓取图标，即可通过拖拽调整事件的先后顺序。

### 5.2 蒙太奇看板 (Montage Board)

*   **场景化组织**：将事件以场景为单位进行组织，适合梳理特定场景下的事件发生顺序。
*   **拖拽关联**：您可以将时间轴中的事件拖拽到不同的场景中进行关联。

### 5.3 标签系统 (Tagging System)

*   **标签管理**：
    *   点击时间轴筛选栏中的 **"Manage Tags"** 按钮，打开标签管理界面。
    *   **创建/编辑**：您可以创建新标签并为其指定颜色。
    *   **删除确认**：删除标签时，系统会显示**内联确认界面**（Confirm/Cancel），操作流畅且不会打断您的工作流。
*   **事件关联**：
    *   在添加新事件或编辑现有事件时，可以直接从标签列表中选择多个标签进行关联。
    *   事件卡片上会显示已关联的标签及其颜色，方便快速识别。

## 6. 任务与日历 (Deadline Tab)

*   **实时日期**：日历视图会自动定位到**当前日期**，方便您实时追踪进度。
...
*   **侧边栏菜单**：
    *   Works 侧边栏在电脑端默认隐藏，但在移动端展开时会显示完整内容（包括作品标题）。
    *   点击左上角的 **菜单按钮 (☰)** 可展开侧边栏。
...
*   **交互优化**：
    *   **防止缩放**：在编辑透镜 (Lens) 内的文字时，系统会自动优化字号，防止移动端浏览器自动缩放屏幕，确保编辑体验流畅。
    *   **宽度一致**：透镜块与普通文本块在移动端保持完全一致的宽度，视觉更加整齐。
    *   **锁定横向滚动**：编辑器禁用了左右滑动，仅允许上下滚动，避免误触导致页面偏移。
*   **返回导航**：
